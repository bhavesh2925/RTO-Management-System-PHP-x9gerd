...
if (strlen($_SESSION['rtomsuid']==0)) {
     ...
     if(isset($_POST['submit']))
  {
    ...
    $stateid=$_POST['state'];
$cityid=$_POST['city'];
    $learninglicno=$_POST['learninglicno'];
...
$photo=md5($photo).time().$extension;
 move_uploaded_file($_FILES["photo"]["tmp_name"],"dlimages/".$photo);
 $query=mysqli_query($con,"insert into tblldicence(LearningLicenceno,ApplicationNumber,UserID,StateID,CityID,ProfilePic,FathersName,DateofBirth,EducationQualification,LicenceType,PermanentAddress,CommunicationAddress,BloodGroup) value('$learninglicno','$applicationnum','$uid','$stateid','$cityid','$photo','$fathersname','$dob','$qualification','$licencetype','$peradd','$commadd','$bloodgroup')");
    if ($query) {
$ret=mysqli_query($con,"select ApplicationNumber from tblldicence  where UserID='$uid'");
$result=mysqli_fetch_array($ret);
$_SESSION['appno']=$result['ApplicationNumber'];
 echo "<script>window.location.href='thank-you1.php'</script>";  
  }
  else
    {
...
<html>
<head>
<title>RTO Management System || Driving License</title>
...
<!-- mail -->
	<div class="mail">
		<div class="container">
			...
			<p class="quia">Fill The Form For Driving License</p>
			
			<div class="col-md-12 wthree_contact_left">
				<h4>Learing License</h4>
				<form action="#" method="post" enctype="multipart/form-data">
					<div class="col-md-6 wthree_contact_left_grid">
						<label>Learing License Number</label>
						<input type="text" name="learninglicno" required="" placeholder="Learing License Number" pattern="[0-9]+" title="only numbers"><br>
						...
						<input type="text" name="qualification" placeholder="Education Qualification*" required=""><br>
						<label>Blood Group</label>
						<input type="text" name="bloodgroup" placeholder="Blood Group*">
				
						<label style="padding-top: 20px;">Licence Type</label>
...
<?php } ?>
        ...
        <br>
						...
						<textarea type="text" name="peradd" placeholder="Permanent Address*" required=""></textarea>
